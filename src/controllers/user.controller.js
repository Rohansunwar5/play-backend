...
const getUserChannelProfile = asyncHandler(async(req,res) => {
  const {username} = req.params  // as this is taken from thr url not from the body
  ...
  const channel = await User.aggregate([
    ...
    {
      $lookup : {
        from : "subscriptions", //  coming from subscription model or db , its stored in plural that's why we are using this field name 
        localField: "_id",
    ...
    {
      $addFields: { // additional fields add up
          subscribersCount: {
            $size:"$subscribers"
          },
    ...
    {
      $project: {// projection  removes unnecessary data, give only selected data 
        fullName: 1,
        username: 1,
        subscribersCount: 1,
...
const getWatchHistory = asyncHandler(async (req, res) => {
  const user = await User.aggregate([
    {
      $match: { // creating moongose  match operator to filter the documents based on provided condition
